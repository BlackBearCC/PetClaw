...
const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "petclaw.json");
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      ...
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "petclaw.json"),
        );
    ...
    it("rejects oversized include files", async () => {
      ...
      try {
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "petclaw.json")),
