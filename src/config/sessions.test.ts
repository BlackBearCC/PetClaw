...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ PETCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from PETCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { PETCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when PETCLAW_STATE_DIR differs from stored paths", () => {
